...
| --stanza \[Stanza ID\] | JavaScript版のStanzaファイル一式を生成する。tsコマンドがインストールされており、tsコマンドがインストールされているディレクトがPATH環境変数に設定されている必要がある。 |
| --stanza_rb \[Stanza ID\] | Ruby版のTogoStanzaをセットアップし、Stanzaファイル一式を生成する。TogoStanzaのgemがインストールされている必要がある。また、togostanzaコマンドがPATH環境変数に設定されている必要がある。 |
| --grasp | Graspの設定ファイル（GraphQLのスキーマファイルとクエリファイル）を生成する。Graspの設定ファイルは、カレントディレクトリ配下の「grasp/設定ファイルディレクトリ名」というディレクトリに生成される。 |
| --shex | ShExを標準出力に出力する。 |
